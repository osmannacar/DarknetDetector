                           ...
                           int output_height)
{
    ...
    //assign classes size
    m_classes = m_class_names.size();

    m_net = load_network(const_cast<char *>(net_cfg_file.c_str()), const_cast<char *>(weight_cfg_file.c_str()), 0);
    if (!m_net) {
...
bool Detector::impl::detect(const Image & image)
{
    ...
    if (m_nms > 0)
        do_nms_sort(m_dets, m_nboxes, m_classes, m_nms);
...
bool Detector::impl::get_detections(std::vector<Detection>& detections)
{
    ...
    for (int i = 0; i < m_nboxes; ++i) {
        float prob;
        int class_index = max_index(m_dets[i].prob, m_classes);

        if (class_index >= (int)(m_class_names.size())) {
            EPRINTF("Class index exceeds class names list, probably the model does not match the names list, class_index:%d\n", class_index);
            return false;
        }
